  ...
  const fetchChallenges = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `https://eco-track-server-pied.vercel.app/challenges?${params.toString()}`
      );
          ...
          {loading && !challenges.length ? (
            ...
            <div className="col-span-full text-center py-20">
              ...
              <button
                ...
                className="btn btn-ghost text-green-600 mt-2"
              >
                Clear All Filters
              </button>
            </div>
            ...
            challenges.map((challenge) => (
              <div
                key={challenge._id}
                className="card bg-white shadow-lg hover:shadow-2xl rounded-3xl overflow-hidden transform hover:-translate-y-2 transition-all duration-300"
              >
                <figure className="relative h-52">
                  ...
                  />
                  <div className="absolute top-3 left-3 bg-green-600/90 backdrop-blur-md text-white px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider">
                    {challenge.category}
                  </div>
                </figure>

                <div className="card-body p-6">
                  <h2 className="card-title text-green-900 text-lg font-bold line-clamp-1">
                    {challenge.title}
                  </h2>
                  ...
                  <div className="card-actions mt-6">
                    <Link
                      ...
                      className="btn btn-primary w-full bg-green-600 border-none hover:bg-green-700 text-white rounded-xl shadow-md"
                    >
                      View Details
                    </Link>
                  </div>
            ...
            {Array.from({ length: totalPages }, (_, i) => (
              <button
                ...
                className={`btn btn-sm ${
                  page === i + 1
                    ? "btn-active bg-green-600 border-none text-white"
                    : "btn-outline"
                }`}
              >
                {i + 1}
